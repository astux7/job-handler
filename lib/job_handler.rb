...
class JobHandler
  ...
  def check_lefts
    raise "Jobs has loops or bad dependecies" unless unique_jobs == handled_jobs
  end
  ...
  def handled_jobs
    @output.delete(' ').split(//).uniq.count
  end
  ...
  def add_dependency_to_new_job(job_unit) 
    if @job_container.last.has_dependency
      ...
      @job_container.last.follows_after_index = index.nil? ? @job_container.count : index
  ...
  def add_dependency_to_found_job(job_unit,find_object) 
    if @job_container[find_object].has_dependency
      index = job_container.find_index{|item| item.name == job_unit[3]} 
      @job_container[find_object].follows_after_index = index.nil? ? @job_container.count : index
      @job_container <<  Job.new(job_unit[3],false, @job_container.count)  if index.nil?
    end
  end
  ...
  def search_dependency_sequence(job)
    index =  @job_container.find_index{|j| j.follows_after_index == job.index}
    unless index.nil?
